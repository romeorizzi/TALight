...
def try_to_convert(tk, tk_type, regex, TAc=None):
    if tk_type == bool:
        if tk.upper() in {'0','FALSE','F'}:
            return 0
        if tk.upper() in {'1','TURE','T'}:
            return 1
        return None
    elif tk_type == int:
        try: 
        ...
        if input_line.strip() == "":
            if not empty_lines_are_comments:
                if (num_tokens and num_tokens > 1) or token_type != str:
                    ...
                    tokens = [""] 
                    break
        elif input_line[0] not in comment_lines_start_with:
            tokens = input_line.split(sep) 
            break
    if num_tokens != None and len(tokens) != num_tokens:
        for out in [stdout, stderr]:
            ...
            if sep != None:
               ...
               TAc.print(f"Runs of space characters are what splits up the input line into tokens.", "yellow", file=out)                
        exit(0)
    vals = []
        ...
        if vals[-1] == None:
            if token_type == str:
                for out in [stdout, stderr]:
                    TAc.print("Input error from the problem-solver:", "red", ["bold"], file=out,end=" ")
                    if num_tokens != 1:
                        print(f"Input error from the problem-solver:  when it got to the {i}-th token:\n'{token}'\nof your input line\n'{input_line}'\nthe server was actually expecting a string matching the regex:\n{regex}\nbut your token does not comply this regex.\n", file=out)
                    else:
                        print(f"Input error from the problem-solver:  the server was actually expecting a string matching the regex:\n{regex}\nbut the string you entered does not comply this regex. The string you entered is '{input_line}'\n", file=out)
                    if line_explained != None:
                        print(f"In practice, the expected line should encode {line_explained}", file=out)
                    if regex_explained != None:
                        if num_tokens == 1:
                            print(f"Your line should encode {regex_explained}\nYour line should comply the regex: {regex}", file=out)
                        else:
                            print(f"Each token of your line should encode {regex_explained}\nEach token should comply the regex: {regex}", file=out)
                ...
                for out in [stdout, stderr]:
                    ...
                    TAc.print(f"Where the server was expecting a token of type {token_type}.\n", "red", file=out)
                    TAc.print("I am dropping the communication because of violation of the intended protocol between problem solver and problem maker.", "yellow", file=out)
            exit(0)
