...
class CampoMinato:
    ...
    def __init__(self, M=None, falling_columns=None, game=False, i_t=0, j_t=0):
        self.is_game = game
        if M is None:
            self.m, self.n = map(int, input().strip().split())
            self.i_t, self.j_t = map(int, input().strip().split())
            self.M = []
    ...
    def as_str_through(self, with_m_n=True):
        ret = f"{str(self.m)} {str(self.n)}\n" if with_m_n else ""
        ret += f"{str(self.i_t)} {str(self.j_t)}\n"
        ret += "".join(map(lambda x: "." if x else "#", self.M[0]))
        for i in range(1, self.m):
            ret += "\n" + "".join(map(lambda x: "." if x else "#", self.M[i]))
        return ret
    ...
    def from_string_through(self, string):
        self.M = []
        self.i_t, self.j_t = map(int, string.split("\n")[1].split())
        for line in string.split("\n")[2:]:
            self.M.append(list(map(lambda x: True if x == "." else False, line)))
        self.m = len(self.M)
        self.n = len(self.M[0])
    ...
    def num_path_through_ric_memo(self, i, j):
        assert 0 <= i < self.m
        assert 0 <= j < self.n
        if not self.M[i][j]:
            return 0
        if i == 0 and j == 0:
            return 1
        M, N = self.m, self.n
        self.m = i + 1; self.n = j + 1
        half_path = self.num_paths_from_ric_memo()
        self.m = M; self.n = N
        return half_path * self.num_paths_to_ric_memo(i, j)
