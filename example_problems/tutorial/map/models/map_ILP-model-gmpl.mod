...
set CELL := {Rows, Cols};  # the set of cells comprising the map (which in practice is a square checkboard)    
set CELL_GOOD := { (i,j) in CELL : MAP[i,j] = 0 };
set CELL_BAD := { (i,j) in CELL : MAP[i,j] = 1 };

var dist{CELL} integer, >= 1; # to every cell we associate an integral variable expressing its distance from cell (1,1) in the map
