...
def new_set(seed,source):
    if source=='catalogue':
        ...
        output_filename = f"instance_catalogue_{ENV['instance_id']}.txt"
    else:
...
def what_to_do():
    ...
    if answer_what_to_do[0]=='stop':
        ...
        TAc.print(LANG.render_feedback("algorithm", 'Ok! Ecco una proposta per il livello successivo: \nProva a scrivere un algoritmo ricorsivo che calcoli per te il massimo di un insieme finito'),  "yellow", ["bold"])
        exit(0)
