...
if parameter=='parameter':
    ...
    if user_max=='none' or user_max==None:
        ...
        if user_max<min_comparison:
            TAc.print(LANG.render_feedback("error", f'Vedi, {user_max} e\' troppo piccolo, non appartiene all\'insieme, quindi non puo\' essere un massimo.'),  "red", ["bold"])
            exit(0)
        else:
            user_k=(user_max-1)/int(condition[0]) if '+' in condition else (user_max)/int(condition[0])
            if not user_k.is_integer():
                TAc.print(LANG.render_feedback("error", f'Vedi, {user_max} non appartiene all\'insieme perche\' non rispetta la condizione {condition} per qualche k naturale... quindi non puo\' essere un massimo'),  "red", ["bold"])
                exit(0)
            else:
                x_proposal=int(eval(condition, {'k':user_k+random.randint(1,3)}))
                TAc.print(LANG.render_feedback("error", f'Vedi, {x_proposal} e\' maggiore del tuo {user_max} ma e\' nell\'insieme, quindi {user_max} non puo\' essere un massimo'),  "red", ["bold"])
                exit(0)
    ...
    if sup==inf: # l'insieme non è limitato superiormente
        if user_max=='none' or user_max==None:
            ...
            TAc.print(LANG.render_feedback("error", f'No, questo insieme e\' illimitato superiormente... quindi non puo\' avere un massimo.'),  "red", ["bold"])
            exit(0)
        ...
        if max==None: # non esiste il massimo
            if user_max=='none' or user_max==None: # l'utente risponde giusto
                ...
                if user_max_comparison==sup or user_max_comparison>sup or ((user_max_comparison<min) if min!=None else None):
                    TAc.print(LANG.render_feedback("error", f'Vedi, {user_max} non appartiene all\'insieme, quindi non puo\' essere un massimo.'),  "red", ["bold"])
                    exit(0)
                elif user_max_comparison<sup:
                    TAc.print(LANG.render_feedback("error", f'Vedi, {(user_max+sup)/2} e\' maggiore del tuo {user_max}, ma e\' nell\'insieme... quindi {user_max} non puo\' essere un massimo.'),  "red", ["bold"])
                    exit(0)
        else: # eiste il massimo
            if user_max=='none' or user_max==None: # l'utente dice che non esiste il massimo
                TAc.print(LANG.render_feedback("error", f'Vedi, {max} appartiene all\'insieme e non esistono altri numeri maggiori di lui, quindi il massimo esiste ed e\' {max}!'),  "red", ["bold"])
                exit(0)
            else:
                # print(f'min {min}, user max {user_max}, max {max}')
                if user_max==max:
                    TAc.print(LANG.render_feedback("correct", f'Ottimo! {max} appartiene all\'insieme e non esistono altri numeri maggiori di lui, quindi e\' un massimo!'), "green", ["bold"])
                    exit(0)
                else:
                    user_max_comparison=abs(user_max) if power==2 else user_max
                    if (min!=None and user_max_comparison<min) or user_max_comparison>max:
                        if power!=1:
                            TAc.print(LANG.render_feedback("error", f'Vedi, {user_max} non appartiene all\'insieme (perche\' {user_max}^{power}={user_max**power}), quindi non puo\' essere un massimo.'),  "red", ["bold"])
                        else:
                            TAc.print(LANG.render_feedback("error", f'Vedi, {user_max} non appartiene all\'insieme, quindi non puo\' essere un massimo.'),  "red", ["bold"])
                        exit(0)
                    else:
