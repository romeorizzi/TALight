...
if user_N<=0 or int(user_N)!=user_N:
    ...
    exit(0)
user_N=int(user_N)
for i in range (user_N+1,user_N+1000,10):
    if abs(successione.subs(n,i)-succ_limit)>=epsilon_value:
        ...
        exit(0)
TAc.print(LANG.render_feedback("correct-n_0", f'Molto bene! In effetti non sono riuscito a trovare nessun valore per n>{user_N} che smentisca la condizione |{successione}-{succ_limit}|'), "green",["bold"])
exit(0)