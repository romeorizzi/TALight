...
def check_command_subset(stringa,n):
    ...
    if end_value>n or start_value>n:
        ...
        exit(0)
def check_command_confronto(user_sol,n):
    arg_1,arg_2=ll.args_confronto(user_sol)
    def check_argument(argument,n):
        if not argument[0]=='s' or not (argument[1:].isdigit() or eval(argument[1:],{'n':n})<=n):
...
def check_max_after_return(indices_set,user_sol,max_elem,n):
    if len(indices_set)>1:
        ...
        exit(0)
    if user_sol[7]=='s' and (user_sol[8:].isdigit() or eval(user_sol[8:],{'n':n})<=n):
...
if cardinality!='open':
    ...
    TAc.print(LANG.render_feedback("inductive-step", f'Potrai disporre di massimo 8 righe; una volta terminato scrivi "return sj" (dove sj e` il massimo per un certo j) e capiro` che hai finito.'), "yellow", ["bold"])
    ...
    while (not 'return' in user_sol) and algo_index<=8 and len(indices_set)>1:
        ...
        if 'max_in_sottoinsieme' in user_sol:
            check_command_subset(user_sol,n)
                ...
                check_command_confronto(user_sol,n)
    ...
    if 'return' in user_sol:
        check_max_after_return(indices_set,user_sol,max_elem,n)
    elif algo_index>=9:
    ...
    TAc.print(LANG.render_feedback("set-explain", 'Consideriamo l\'insieme S della forma S={s1,s2,s3,...,sn} con n>1. Qual e` il massimo dell\'insieme S? Per rispondere hai a disposizione ben 2 oracoli: \n- confronto(sj,sk) : confronta due elementi nell\'insieme S, ovvero sj ed sk e restituisce il maggiore dei due \n- max_in_sottoinsieme(j,k) : considera il sottoinsieme proprio di S costituito dagli elementi che vanno da sj fino ad sk (quindi j<=k) e restituisce il massimo tra questi. \nProva a combinare chiamate agli oracoli disponibili in modo da costruire il massimo. Potrai disporre di massimo 8 righe; una volta terminato scrivi "return sj" (dove sj e` il massimo per un certo j) e capiro` che hai finito.'), "yellow", ["bold"])
