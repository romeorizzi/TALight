...
if ENV['value'] == -2: # user conjectures this is a first_to_move_wins position
    if not(f_player_w) and moves==[]:
        TAc.print(LANG.render_feedback("not-a-winning-graph", f'No! Contrary to your conjecture, the graph \'{ENV["graph"]}\' is NOT a winning one.'), "red")
        ...
        TAc.print(LANG.render_feedback("ok-winning-graph", f'Ok! We agree with your conjecture that the graph \'{ENV["graph"]}\' is a winning one.'), "green", ["bold"])

if ENV['value'] == -1: # user conjectures this is a first_to_move_loses position
    if f_player_w or moves!=[]:
        TAc.print(LANG.render_feedback("not-a-lost-graph", f'No! Contrary to your conjecture, the graph \'{ENV["graph"]}\' is NOT a lost one.'), "red")
        ...
        TAc.print(LANG.render_feedback("ok-lost-graph", f'Ok! We agree with your conjecture that the graph \'{ENV["graph"]}\' is a lost one.'), "green", ["bold"])
