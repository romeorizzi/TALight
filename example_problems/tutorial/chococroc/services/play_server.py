...
if ENV['player'] == 1: #se l'utente vuole giocare per secondo
    ...
    if(ENV['watch_value'] == 'watch_winner'):
        if(cl.grundy_val(m,n) == 0):
            TAc.print(LANG.render_feedback("watch-winner-user-after-server", f'watch_winner: you, since ({m}, {n}) is a losing configuration.'), "blue")
        else:
            TAc.print(LANG.render_feedback("watch-winner-server-after-server", f'watch_winner: me, since ({m}, {n}) is a winning configuration.'), "blue")
    elif ENV['watch_value'] == 'num_winning_moves' :
        win_moves = cl.winning_moves(m,n)
        win_moves.discard((None, None))
        if len(win_moves) >= 1:
            TAc.print(LANG.render_feedback("num-winning-moves-n", f'num_winning_moves: for the current configuration ({m}, {n}) the number of winning moves is {len(win_moves)}'), "blue")
        else:
            TAc.print(LANG.render_feedback("num-winning-moves-0", f'num_winning_moves: for the current configuration ({m}, {n}) there are not winning moves'), "blue")
    elif(ENV['watch_value'] == 'list_winning_moves'):
        win_moves = cl.winning_moves(m, n)
        win_moves.discard((None, None))
        if len(win_moves) > 1:
            TAc.print(LANG.render_feedback("list-multiple-winning-moves", f'list_winning_moves: for the current configuration ({m}, {n}) the winning moves are {win_moves}'), "blue")
        elif len(win_moves) == 1:
            TAc.print(LANG.render_feedback("list-one-winning-moves", f'list_winning_moves: for the current configuration ({m}, {n}) the winning move is {win_moves}'), "blue")
        else:
            TAc.print(LANG.render_feedback("list-none-winning-moves", f'list_winning_moves: for the current configuration ({m}, {n}) there are not winning moves'), "blue")
    elif(ENV['watch_value'] == 'watch_grundy_val'):
        TAc.print(LANG.render_feedback("watch-grundy-after-server", f'watch_grundy_val: for the current configuration ({m}, {n}) the grundy value is {cl.grundy_val(m,n)}'), "blue")
...
while True:
    ...
    if(ENV['watch_value'] == 'watch_winner'):
        if(cl.grundy_val(m, n) > 0):
            TAc.print(LANG.render_feedback("watch-winner-user-after-user", f'watch_winner: you, since ({m}, {n}) is a winning configuration.'), "blue")
        else:
            TAc.print(LANG.render_feedback("watch-winner-server-after-user", f'watch_winner: me, since ({m}, {n}) is a losing configuration.'), "blue")
    elif ENV['watch_value'] == 'num_winning_moves' :
        win_moves = cl.winning_moves(m, n)
        win_moves.discard((None, None))
        if len(win_moves) >= 1:
        ...
        TAc.print(LANG.render_feedback("watch-grundy-after-user", f'watch_grundy_val: for the current configuration ({m}, {n}) the grundy value is {cl.grundy_val(m, n)}'), "blue")
    ...
    if(ENV['watch_value'] == 'watch_winner'):
        if(cl.grundy_val(new_m,new_n) == 0):
            TAc.print(LANG.render_feedback("watch-winner-user-after-server", f'watch_winner: you, since ({new_m}, {new_n}) is a losing configuration.'), "blue")
        else:
            TAc.print(LANG.render_feedback("watch-winner-server-after-server", f'watch_winner: me, since ({new_m}, {new_n}) is a winning configuration.'), "blue")
    elif ENV['watch_value'] == 'num_winning_moves' :
        win_moves = cl.winning_moves(new_m,new_n)
        win_moves.discard((None, None))
        if len(win_moves) >= 1:
            TAc.print(LANG.render_feedback("num-winning-moves-n", f'num_winning_moves: for the current configuration ({new_m}, {new_n}) the number of winning moves is {len(win_moves)}'), "blue")
        else:
            TAc.print(LANG.render_feedback("num-winning-moves-0", f'num_winning_moves: for the current configuration ({new_m}, {new_n}) there are not winning moves'), "blue")
    elif(ENV['watch_value'] == 'list_winning_moves'):
        win_moves = cl.winning_moves(new_m, new_n)
        win_moves.discard((None, None))
        if len(win_moves) > 1:
            TAc.print(LANG.render_feedback("list-multiple-winning-moves", f'list_winning_moves: for the current configuration ({new_m}, {new_n}) the winning moves are {win_moves}'), "blue")
        elif len(win_moves) == 1:
            TAc.print(LANG.render_feedback("list-one-winning-moves", f'list_winning_moves: for the current configuration ({new_m}, {new_n}) the winning move is {win_moves}'), "blue")
        else:
            TAc.print(LANG.render_feedback("list-none-winning-moves", f'list_winning_moves: for the current configuration ({new_m}, {new_n}) there are not winning moves'), "blue")
    elif(ENV['watch_value'] == 'watch_grundy_val'):
        TAc.print(LANG.render_feedback("watch-grundy-after-server", f'watch_grundy_val: for the current configuration ({new_m}, {new_n}) the grundy value is {cl.grundy_val(new_m,new_n)}'), "blue")
