...
def TALinput_line(tokens_type, regex="^((\S)+)$", regex_explained=None, comment_or_command_lines_start_with='#', last_line_command=None):
    ...
    while True:
        ...
        if len(spoon) == 0:
            print(f"You have entered an unexpected empty line. I assume you want to drop this TALight service call. See you next time ;))")
            exit(0)
        if spoon[0] not in ignore_lines_start_with:
            break
        if spoon[1:].split()[0].upper() == stopping_line:
            return False
    tokens = spoon.split() 
