...
if ENV['risp'] == risp_correct:
    ...
    if ENV['more_or_less_hint_if_wrong']:
        if overflow:
            ...
            if ENV['risp'] < risp_correct:
                TAc.print(LANG.render_feedback("more", f"No. The well-formed tilings of a corridor of dimension 1x{ENV['n']} are strictly more than {ENV['risp']}. If you don't believe this, you can check it out with the service check_solutions_set."), "red", ["bold"])
                if ENV['ok_if_congruent_modulus'] != 0:
                ...
                TAc.print(LANG.render_feedback("less", f"No. The well-formed tilings of a corridor of dimension 1x{ENV['n']} are actually strictly less than {ENV['risp']}. If you don't believe this, you can check it out with the service check_solutions_set."), "red", ["bold"])
    else:
        if ENV['ok_if_congruent_modulus'] != 0:
            TAc.print(LANG.render_feedback("not-congruent", f"No. The number of well-formed tilings of a corridor of dimension 1x{ENV['n']} is NOT congruent to {ENV['risp']} modulo {ENV['ok_if_congruent_modulus']}."), "red", ["bold"])
        else:
