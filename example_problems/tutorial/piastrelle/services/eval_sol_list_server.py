...
for n_tiles in instances:
    ...
    if time > 1:
        if n_tiles > 13:
            TAc.print(LANG.render_feedback("seems-correct-weak", f'# Ok. :) Your solution correctly computes the rank of well formed formulas (checked with formulas up to {n_tiles} pairs of parentheses).'), "green")
        TAc.print(LANG.render_feedback("not-efficient", f'# No. You solution is NOT efficient. When run on your machine, it took more than one second to compute the rank of a well-formed formulas with {n_tiles} pairs of parentheses.'), "red", ["bold"])        
        exit(0)
