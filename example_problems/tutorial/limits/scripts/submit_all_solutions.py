...
Usage: {os.path.basename(sys.argv[0])}  {{ GET_FEEDBACK | SUBMIT }}  [--instances {{ MY_INSTANCES | CATALOGUE }}] [--sol_format] [--inst_format] [--sol_ext] [--inst_ext] [--sol_folder] [--inst_folder]

   where the two alternative options for the obligatory 'mode' argument are:
   * SUBMIT: act a real sumbission! In this case you will be required to introduce your personal token.
   * GET_FEEDBACK [default]: when your intantion is only to get a feedback or validation on your solutions. For this you do not need to be enrolled in the course or exam.

   the two alternative options for the optional 'instances' argument are:
   ...
   * CATALOGUE: when your intention is only to get a feedback or validation of your solutions to the instances in the catalogue. In this case only the solutions whose filename's kernel is a correct instance_id will be sent for evaluation being matched on the instances on the server.
...
if __name__ == "__main__":
    ...
    description=usage,
    epilog="""-------------------""")
    parser.add_argument('mode', type=str, choices=['GET_FEEDBACK', 'SUBMIT'], help='choose one option in the set { GET_FEEDBACK | SUBMIT }. In order to submit you are also asked to supply your personal TALight token', default='GET_FEEDBACK')
    parser.add_argument('--instances', type=str, choices=['MY_INSTANCES', 'CATALOGUE'], help='for this optional argument choose one option in the set { MY_INSTANCES | CATALOGUE }. When you are submitting then this option is forcibly set to CATALOGUE hence twikering this argument is useless. Otherwise, setting this option to MY_INSTANCES (which is also the default value) will refer your solutions to the instances contained into your local {PATH_INSTANCES} folder. This script will then issue an rtal check_sol service request for every (instance_file,solution_file) pair of files whose names match, with instance_file in your local {PATH_INSTANCES} folder and solution_file in your local {PATH_SOLUTIONS} folder.', default='MY_INSTANCES')
    ...
    parser.add_argument('--cert_folder', type=str, help=f'use this optional argument to specify a different name/location for the folder where to collect the certificates for your positive submissions (the default is {PATH_CERTIFICATES}).', default=PATH_CERTIFICATES)
    ...
    if args.mode == 'GET_FEEDBACK' and args.instances == 'MY_INSTANCES':
        list_solutions = [ f for f in os.listdir(args.sol_folder) if ext(f) == SOL_EXT ]
    else:
        list_solutions = [ f for f in os.listdir(args.sol_folder) if ext(f) == SOL_EXT and is_instance_id(kernel(f)) ]
    ...
    def command_submission(solution_file, instance_id):
        return f"rtal connect {PROBLEM_NAME} -x {{PERSONAL_TOKEN}} -o {args.cert_folder} check_sol -fsolution={os.path.join(args.sol_folder,solution_file)} -asol_format={SOL_FORMAT} -ainstance_id={instance_id}"
    ...
    def command_feedback_free_instances(solution_file, instance_file):
        return f"rtal connect {PROBLEM_NAME}  check_sol -fsolution={os.path.join(args.sol_folder,solution_file)} -asol_format={SOL_FORMAT} -finstance={os.path.join(args.inst_folder,instance_file)} -ainstance_format={INST_FORMAT}"
    ...
    for solution_file in list_solutions:
        ...
        elif args.instances == 'CATALOGUE':
            instance_id = int(kernel(solution_file))
            calls_list.append(command_feedback_catalogue(solution_file, instance_id))
        else:
            for instance_file in list_instances:
                if kernel(instance_file) == kernel(solution_file):
                    calls_list.append(command_feedback_free_instances(solution_file, instance_file))
    ...
    print(f"\n{CBOLD}QUADRO DELLE NOSTRE {len(calls_list)} RICHIESTE AL SERVIZIO `rtal connect {PROBLEM_NAME} check_sol`:{CEND}\n")
    go_through_calls(calls_list, dry_run=True, PERSONAL_TOKEN=None, PERSONAL_TOKEN_FOR_DISPLAY='<PERSONAL_TOKEN_NEEDED>')
    print(f"\n{CBOLD}PRESO VISIONE DELLA LISTA DI {len(calls_list)} RICHIESTE AL SERVIZIO `rtal connect {PROBLEM_NAME} check_sol`?{CEND}\n")
    YOUR_PERSONAL_TOKEN=None
    if args.mode == 'SUBMIT':
        print("Trattandosi di sottomissioni, se vuoi procedere devi inserire il tuo TALight personal token")
