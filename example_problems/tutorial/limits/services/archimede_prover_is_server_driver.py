...
def disprove():
    ...
    try:
        ...
        exit(0)
    if x_eval<=0:
        TAc.print(LANG.render_feedback("error", f'hai inserito un valore per x<=0, non posso continuare'), "red", ["bold"])
        exit(0)
    else:
        if x_eval>1:
...
disprove()
TAc.print(LANG.render_feedback("what-to-do", f'\nSe ora vuoi provare a convincermi tu, invertendo i ruoli, ti consiglio il servizio `rtal connect limits archimede_prover_is_user`'),  "white", ["bold"])