...
def check_row_index(index:int,m:int,TAc,LANG):
    if index < 0 or index >= m:
        TAc.print(LANG.render_feedback("row-index-out-of-range", f"Row index {index} falls outside the valid range. The valid range for row inexes is [0,{m-1}].", {"m":m}), "red", ["bold"])
            exit(0)
...
def is_optimal_asteroid_set(m:int,n:int,matrix,asteroids,silent:bool,give_a_better_solution_if_any:bool,TAc,LANG):
    """verifyies whether the asteroids of a given subset are independent. Returns either True or False. When the arg silent=False then it also prints out about its findings."""
    assert is_feasible_asteroid_set(m,n,matrix,beams,silent=True,TAc=TAc,LANG=LANG)
    if len(asteroids)==opt_val(m,n,matrix):
        ...
        if give_a_better_solution_if_any:
            bigger_set = max_match_bip(m,n,matrix)[:len(asteroids)+1]
            TAc.print(LANG.render_feedback("bigger-asteroid-set", "A bigger set of independent asteroids could be: {bigger_set}"), "red", ["bold"])            
        return False
