...
def GenerateConnectedGraph(n, m):
	g = Graph(n)
...
def GenerateNonConnectedGraph(n, m):
	if(n < 2):
		raise ValueError('Can\'t create a connected graph with a single node') # TODO: gestire dal chiamante
	tmp_n = int(n/2)
	tmp_m = int(m/2)
	g3 = Graph(n)
	graph_print3 = ""
	...
	if(((tmp_n * (tmp_n-1)) + ((n-tmp_n) * (n-tmp_n-1)) < m)): #casoparticolare
		g1, graph_print1 = GenerateConnectedGraph(n-1,m) 
		...
		g1, graph_print1 = GenerateConnectedGraph(tmp_n, tmp_m)
		g2, graph_print2 = GenerateConnectedGraph(n-tmp_n, m-tmp_m)
...
def GenerateGraph(seed, n, m, is_connected):
	...
	if(m < n-1):
		raise ValueError('Can\'t create a connected graph with m less than n-1') # TODO: gestire dal chiamante


	random.seed(int(shake))
	if(is_connected): # Genero grafo connesso
		g, graph_print = GenerateConnectedGraph(n, m) 
	else: # Genero grafo NON connesso
		g, graph_print = GenerateNonConnectedGraph(n,m)
	
	return g, graph_print, g.listConnectedEdges(), shake
