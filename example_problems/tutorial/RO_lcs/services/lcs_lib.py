...
def dict_of_input(ENV):
    dict_input = {}
    RO.add_ENV_vars(['s','t','sol_type'], dict_input, ENV)
    RO.add_ENV_vars(['start_with','end_with'], dict_input, ENV, lambda x : x !='-1')
    RO.add_ENV_vars(['forbidden_s_interval_first_pos','forbidden_s_interval_last_pos'], dict_input, ENV, lambda x : x !='X')
    if ENV['sol_type'] in ['opt_sol','opt_sol_with_val','all']:
...
def check_request_consistency(ENV):
    s = ENV["s"]
    t = ENV["t"]
    assert ENV["start_with"] == '*' or (ENV["start_with"] in s and ENV["start_with"] in t)
    assert ENV["end_with"] == '*' or (ENV["end_with"] in s and ENV["end_with"] in t)
    assert ENV["forbidden_s_interval_first_pos"] < min(len(s),len(t))
    assert ENV["forbidden_s_interval_last_pos"] < min(len(s),len(t))
    first_pos = ENV["pesi"]
