...
if ENV['n']!='lazy':
    ...
    TAc.print(LANG.render_feedback("gimme-m", "Insert the number of rows. Parameter n = "), "yellow", ["bold"])
    n=TALinput(
        int,
        num_tokens=1,
        TAc=TAc
...
line = ['0'] * n
TAc.print(LANG.render_feedback("left-margin", f"Insert your tiling line by line ({m} lines of length {n} over the alphabet {{N,E,S,W,0,1,2,3,4}} as explained by the synopsis service and other documentation of the exercise)."), "yellow")
for r in range(m):
    prev_line = line
    line = TALinput(
        str,
