  ...
  if not weighted:
    if int(size_sol) <= upper_bound and int(size_sol) >= lower_bound:
      if int(size_sol) == lower_bound and int(size_sol) == upper_bound:
        TAc.OK()
        TAc.print(LANG.render_feedback("goal", f'You reached the goal! The size of the vertex cover is exactly {size_sol}!'), "green", ["bold"], flush=True, end='')
        goal = True
      else:
    ...
    if weight_sol <= upper_bound and weight_sol >= lower_bound:
      if weight_sol == lower_bound and weight_sol == upper_bound:
        TAc.OK()
        TAc.print(LANG.render_feedback("goal-w", f'You reached the goal! The minimum weight of the vertex cover is exactly {weight_sol}!'), "green", ["bold"], flush=True, end='')
        goal = True
      else:
