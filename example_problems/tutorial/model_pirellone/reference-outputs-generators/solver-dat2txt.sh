...
if [ -x ${prefix_solver}_dat2txt.bin ]; then
    ${prefix_solver}_dat2txt.bin < $@
    exit 0
fi 
if [ -x ${prefix_solver}_dat2txt.py ]; then
    ${prefix_solver}_dat2txt.py < $@
    exit 0
fi
if [ -e ${prefix_converter}_dat2txt.cpp ]; then
    if [ ! -x ${prefix_converter}_dat2txt.bin ]; then
        g++ -Wall --pedantic -O2 -o ${prefix_converter}_dat2txt.bin ${prefix_converter}_dat2txt.cpp
    fi 
fi 
...
solver=""
if [ -x ${prefix_converter}_dat2txt.bin ]; then
    converter=${prefix_converter}_dat2txt.bin
fi 
if [ -x ${prefix_converter}_dat2txt.py ]; then
    converter=${prefix_converter}_dat2txt.py
fi 
if [ -x ${prefix_solver}_txt2txt.bin ]; then
    solver=${prefix_solver}_txt2txt.bin
fi 
if [ -x ${prefix_solver}_txt2txt.py ]; then
    solver=${prefix_solver}_txt2txt.py
fi 
...
if [ -e ${fastest_model} ]; then
#    echo "glpsol -m ${fastest_model} -d $@ > /dev/null"
    glpsol -m ${fastest_model} -d $@ > /dev/null
    cat output.txt
    rm -f output.txt 
    exit 0
fi 
