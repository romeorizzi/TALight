...
class InsertSortingMachine:
    ...
    def compare_what_in_tmp_buffer_with_what_in_pos(self, i: int):
        if self.tmp_buffer is None:
            print("Ahi, my problem-solver bot is comparing an element with an empty tmp_buffer. I expect complaints from the checking server.")
        print(f"#LOG_compare_what_in_tmp_buffer_with_what_in_pos {i} <outcome>")

    def output_final_array(self):
        print(f"#LOG_output_final_array <outcome>")
...
if __name__ == "__main__":
    ...
    n_ordered = 0
    while n_ordered < N:
        SM.load_next_input_element_in_tmp_buffer(n_ordered)
        pos_cmp = n_ordered-1
        while 0 <= pos_cmp and SM.compare_what_in_tmp_buffer_with_what_in_pos(pos_cmp):
