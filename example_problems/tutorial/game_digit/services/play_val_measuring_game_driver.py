...
while True:
    ...
    if ENV['opponent'] == 'computer':
        ...
        if number!=new_number and nim==new_nim:
            TAc.print(LANG.render_feedback("numb-server-move-play-val", f'# My move is from conf <game_digit({new_number}) + nim({new_nim})> to conf <game_digit({number}={new_number}-{new_number-number}) + nim({nim})> obtained from the previous one by subtracting from {new_number} one of its non-zero digits ({new_number-number}).'), "green", ["bold"])
        elif number==new_number and nim!=new_nim:
            TAc.print(LANG.render_feedback("numb-server-move-play-val", f'# My move is from conf <game_digit({new_number}) + nim({new_nim})> to conf <game_digit({number}) + nim({nim}={new_nim}-{new_nim-nim})> obtained from the previous one by subtracting from {new_nim} a certain number ({new_nim-nim}).'), "green", ["bold"])
    else:
