...
class BubbleSortMachine:
    ...
    # Input vettore
    def input_array(self, vector: list, expected_log: str = None):
        if vector is None or len(vector) == 0:
            self.BOT.print("Ahi, the operator required to insert a non existent working array.")
        self.working_array = vector
        self.console(f"LOG_input_array ({len(vector)}: {' '.join(map(str,vector))})", expected_log)

    # Display vettore
    def display_array(self, expected_log: str = None):
        if self.working_array is None or len(self.working_array) == 0:
            ...
            return None
        self.console(f"LOG_display_array ({len(self.working_array)}: {' '.join(map(str,self.working_array))})", expected_log)
    ...
    def output_array_after_one_single_pass(self, expected_log: str = None):
        self.console(f"LOG_output_array_after_one_single_pass ({len(self.working_array)}: {' '.join(map(str,self.working_array))})", expected_log)

    # Sort 2 consecutive elements
    def sort_consecutive_pair(self, first_position: int, expected_log: str = None):
        ...
        self.console(f"LOG_sort_pair_of_consecutive_elements_at_pos {first_position} {second_position}", expected_log)
        
    # Swap consecutive elements
    def swap_consecutive_elements(self, first_position: int, expected_log: str = None):
        second_position = first_position + 1
        ...
        self.console(f"LOG_swap_consecutive_elements {first_position} {second_position}", expected_log)
    
    # Compare consecutive elements
    def compare_consecutive_elements(self, first_position: int, expected_log: str = None):
        ...
        if self.working_array[first_position] < self.working_array[second_position]:
            self.console(f"LOG_compare_consecutive_elements {first_position} (<) {second_position}", expected_log)
            return True
        self.console(f"LOG_compare_consecutive_elements {first_position} (>=) {second_position}", expected_log)
        return False
