...
if ENV["risp"] == risp_correct:
    if not ENV["silent"]:
        ...
        if overflow:
            ...
            TAc.print(LANG.render_feedback("ok-equal", f'♥  We agree. There are precisely {ENV["risp"]} feasible paths in a triangle with {ENV["n"]} rows.'), "green", ["bold"])
    exit(0)

# NOW DEALING WITH WRONG ANSWER:
if ENV["ok_if_congruent_modulus"] == 0:
    TAc.print(LANG.render_feedback("not-equal", f'No. The feasible paths in a triangle with {ENV["n"]} rows are not {ENV["risp"]}.'), "red", ["bold"])
else:
    TAc.print(LANG.render_feedback("not-congruent", f'No. The number of feasible paths in a triangle with {ENV["n"]} rows is NOT congruent to {ENV["risp"]} modulo {ENV["ok_if_congruent_modulus"]}.'), "red", ["bold"])
...
if ENV["more_or_less_hint_if_wrong"]:
    if ENV["ok_if_congruent_modulus"] == 0 or not overflow:
        if ENV["risp"] < risp_correct:
            TAc.print(LANG.render_feedback("more", f'Indeed, the feasible paths in a triangle with {ENV["n"]} rows are strictly more than {ENV["risp"]}. If you do not believe this, you can check it out with the service check_sol_set.'), "red", ["bold"])
            if ENV["ok_if_congruent_modulus"] != 0:
                TAc.print(LANG.render_feedback("why_pertinent", f'And be told that the feasible paths in a triangle with {ENV["n"]} rows are anyhow strictly less than {ENV["ok_if_congruent_modulus"]}, which is the value of the parameter "ok_if_congruent_modulus" for the current call to this service, as you can always check in the opening message.'), "red")
        else:
            TAc.print(LANG.render_feedback("less", f'Indeed, the feasible paths in a triangle with {ENV["n"]} rows are actually strictly less than {ENV["risp"]}. You can check this out through the service check_sol_set.'), "red", ["bold"])
    else:
