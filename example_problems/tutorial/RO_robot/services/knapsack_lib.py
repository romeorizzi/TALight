...
def check_request_consistency(ENV):
    ...
    for ele in ENV["elementi_obbligati"]:
        if ele not in ENV["elementi"]:
            print(f'Errore: the element {ele} containd in the list `elementi_obbligati` is not contained in the list `elementi` = {ENV["elementi"]}.\nIndeed, `elementi_obbligati` = {ENV["elementi_obbligati"]}.')
            exit(0)
    for ele in ENV["elementi_proibiti"]:
        if ele not in ENV["elementi"]:
            print(f'Errore: the element {ele} containd in the list `elementi_proibiti` is not contained in the list `elementi` = {ENV["elementi"]}.\nIndeed, `elementi_proibiti` = {ENV["elementi_proibiti"]}.')
            exit(0)
