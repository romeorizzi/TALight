...
def verif_knapsack(task_number,elements:List[str],weights:List[int],vals:List[int],Capacity:int, answ, pt_tot=None,pt_formato_OK=None,pt_feasibility_OK=None,elementi_proibiti:List[str]=[],elementi_obbligati:List[str]=[]):
    ...
    if answ['sol_type'] in ["opt_val","opt_sol_with_val"]:
        if type(answ['opt_val']) != int:
            feedback_summary += f"formato di `{answ['name_of_opt_val']}`: "+SEF.colored(f"NO{SEF.new_line}", "red", ["bold"])
        ...
        if answ['sol_type'] == "opt_val":
            feedback_summary += f"formato di `{answ['name_of_opt_val']}`: "+SEF.colored(f"OK [{pt_formato_OK} safe pt]{SEF.new_line}", "green", ["bold"])
            return SEF.evaluation_format(task_number, feedback_summary, f"Come `{answ['name_of_opt_val']}` hai immesso un intero come richiesto."+SEF.colored(f"{SEF.new_line}Nota:", "cyan", ["bold"])+" Ovviamente durante lo svolgimento dell'esame non posso dirti se l'intero immesso sia poi la risposta corretta, ma il formato è corretto.", pt_tot,pt_safe=pt_formato_OK,pt_out=0)
        else:
            feedback_summary += f"formato di `{answ['name_of_opt_val']}`: "+SEF.colored(f"OK{SEF.new_line}", "green", ["bold"])

    if answ['sol_type'] in ["opt_sol","opt_sol_with_val"]:
        if type(answ['opt_sol']) != list:
            feedback_summary += f"formato di `{answ['name_of_opt_sol']}`: "+SEF.colored(f"NO{SEF.new_line}", "red", ["bold"])
        ...
        feedback_summary += f"formato di `{answ['name_of_opt_sol']}`: "+SEF.colored(f"OK [{pt_formato_OK} safe pt]{SEF.new_line}", "green", ["bold"])
