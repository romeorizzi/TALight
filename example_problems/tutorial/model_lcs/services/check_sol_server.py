...
if ENV['instance_spec'] == 'random':
    ...
    if ENV['sol_style'] == 'subsequence':
        ...
        if ll.check_sol(TAc, LANG, ENV, instance_with_solution[2], instance_with_solution[0], instance_with_solution[1]):
    ...
    if ENV['sol_style'] == 'annotated_subseq':
        ...
        if ll.check_sol(TAc, LANG, ENV, user_sol, instance_with_solution[0], instance_with_solution[1]):
            TAc.print(LANG.render_feedback("correct-sol", 'Your solution is correct. Well done! You have found the Longest Common Subsequence.'), "green", ["bold"]) 
        exit(0)
