...
prefix_converter=./instance-generators/converter_${problem_name}_txt2dat
...
if [ -x ${prefix_generator}_dat.bin ]; then
    ${prefix_generator}_dat.bin $@
    exit 0
fi 
if [ -x ${prefix_generator}_dat.py ]; then
    ${prefix_generator}_dat.py $@
    exit 0
fi 
...
converter=""
if [ -x ${prefix_generator}_txt.bin ]; then
    generator=${prefix_generator}_txt.bin
fi 
if [ -x ${prefix_generator}_txt.py ]; then
    generator=${prefix_generator}_txt.py
fi 
if [ -x ${prefix_converter}.bin ]; then
