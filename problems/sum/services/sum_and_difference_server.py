...
for _ in range(ENV_num_questions):
    ...
    if a+b > x+y:
       ...
       #English: print(f"No! indeed, {a}+{b}={a+b} < {x+y}.")
        ...
        print_lang(["no_answ", "red", "on_blue", ["blink"]], ["too-apart", "yellow", "on_blue", ["underline"]])        
        #English: print(f"No! indeed, |{a}-{b}|={abs(a-b)} > {x-y}.")
    elif abs(a-b) < x-y:    
        print_lang(["no_answ", "red", "on_blue", ["blink"]], ["too-close", "yellow", "on_blue", ["underline"]])        
        #English: print(f"No! indeed, |{a}-{b}|={abs(a-b)} < {x-y}.")
    else:
